...
var worker = {
  _install: function (bus, coreOptions, userOptions) {
    ...
    // Bind all the functions so users never have to manually call `bind`.
    Object.keys(this).forEach(function (key) {
      if (typeof self[key] === 'function') self[key] = self[key].bind(self);
    });
    ...
    this.bus.on('message', this.callMap);
  },
