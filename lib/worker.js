...
var base = {
  _install: function (bus, coreOptions, userOptions) {
    ...
    worker.bus.on('message', worker.callMap.bind(worker));
  },
  ...
  callMap: function (data) {
    var self = this;
    this.awaitedDones = data.length;

    for (var i = 0; i < data.length; i++) {
      try {
        self.map(data[i], self);
      } catch (error) {
        console.error(error);
   ...
   */
    ...
    if (this.recievedDones === this.awaitedDones) {
      this.awaitedDones = 0;
      this.recievedDones = 0;
      this.bus.send({type: 'doneBatch'});
    }
  }
};
